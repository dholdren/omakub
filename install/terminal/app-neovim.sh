cd /tmp
uname=$(uname -m)
wget -O nvim.tar.gz "https://github.com/neovim/neovim/releases/download/stable/nvim-linux-${uname}.tar.gz"
tar -xf nvim.tar.gz
sudo install nvim-linux-${uname}/bin/nvim /usr/local/bin/nvim
sudo cp -R nvim-linux-${uname}/lib /usr/local/
sudo cp -R nvim-linux-${uname}/share /usr/local/
rm -rf nvim-linux-${uname} nvim.tar.gz
cd -
